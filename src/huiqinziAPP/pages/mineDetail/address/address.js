// pages/mineDetail/address/address.js
import { http, verifyPhone } from '../../../utils/util.js'
...
Page({
  ...
  showAddrChose: function () {
    this.setData({
      showAddrChose: !this.data.showAddrChose
    })
  },
  choseDef: function () {
    this.setData({
      isDefault: this.data.isDefault == "yes" ? "no" : "yes"
    })
  },
  choseAddr: function (e) {
    // debugger
    console.log("choseAddr", e);
    var data = this.data,
      idxes = e.detail.value,
      Cidxes = data.changeIdxs,
      changeIdx = {};
    idxes.map((ele, i) => {
      if (ele != Cidxes[i]) {
        changeIdx = {
    ...
    switch (changeIdx.idx) {
      case 0:
        var p = data.provinces[changeIdx.v],
          cities = Object.keys(data.addresses[p]),
          districts = data.addresses[p][cities[0]];
        this.setData({
          cities: cities,
          districts: districts,
          province: p,
        ...
        break;
      case 1:
        var c = data.cities[changeIdx.v],
          districts = data.addresses[data.province][c];
        this.setData({
          districts: districts,
          city: c,
          district: districts[0]
      ...
      case 2:
        this.setData({
          district: data.districts[changeIdx.v]
        });
        break;
      default:
        break;
    ...
    this.setData({
      changeIdxs: idxes,
      addrInfo: this.data.province + this.data.city + this.data.district
    });
  },
  ...
  submitAddAddr: function () {
    var data = this.data,
      params = {
        useruuid: wx.getStorageSync("uuid"),
        address: data.addrInfo + " " + data.detailAddr,
        contact: data.contact,
        phone: data.phone,
        defaul: data.isDefault
      };
    if (!(data.addrInfo  && data.detailAddr  && data.contact )) {
      wx.showToast({
        title: '地址信息或联系人信息有误！',
        icon: 'none',
        duration: 1000,
    ...
    if (this.data.opr == "edit") {
      params.addressuuid = this.data.addressuuid;
      http('/appaddress/update', params, 'put').then(res => {
        wx.showToast({
          title: '地址修改成功！',
          icon: 'success',
          duration: 1000,
      ...
      http('/appaddress/add', params, 'post').then(res => {
        wx.showToast({
          title: '添加地址成功！',
          icon: 'success',
          duration: 1000,
  ...
  onLoad: function (options) {
    this.setData({
      provinces: Object.keys(this.data.addresses)
    });
    console.log(options)
    if (Object.keys(options).length == 5) {
      this.setData({
        ...
        phone: options.phone,
        addrInfo: options.address.split(" ")[0],
        detailAddr: options.address.split(" ")[1],
        isDefault: options.defaul,
        addressuuid: options.addressuuid
      });
    } else {
      this.setData({
        opr: "add"
